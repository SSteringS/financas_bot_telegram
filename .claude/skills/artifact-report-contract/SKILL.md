...
- Writing or updating a plan, status, review, QA result, code documentation, feature backlog, or technical debt register.
...
- Deciding where a file is saved or how it is named — that is defined in `.github/instructions/artifact-placement-and-naming.instructions.md`.
...
2. Feature folder and `TASK-ID`.
3. Whether an artifact must be persisted or only a chat response is expected.

## If information is missing

- Missing feature or `TASK-ID`: stop and ask; never write an artifact with a placeholder identifier.
...
6. Save the artifact using the placement instruction rules.
...
- Backend and reviewer must fill `Technical Debt Identified`; the planner consolidates it into `pendencias-tecnicas.md`.
- `intervencoes.md` is created only when the first human intervention happens in the feature; never create it empty.
...
- Artifact structure, ownership, and cross-agent field compatibility: [references/artifact-contract.md](references/artifact-contract.md).
...
- Stop if the feature or `TASK-ID` is not confirmed.
