...
Bot de finanças para Telegram com frontend web. Monorepo com múltiplos módulos e instâncias de Codex colaborando em paralelo.

## Instâncias do Codex — quem faz o quê

| Instância | Worktree (pasta no disco) | Branch | Responsabilidade |
|---|---|---|---|
| Codex do **back** | `C:\Users\satya\src\financas_bot_telegram` | `feature/be-NNN-*` / `fix/NNN-*` / `hotfix/NNN-*` | API REST, domínio, banco, deploy. Pasta de código: `financas_bot_telegram/` + `infra/` + `finbot.service` + `.github/workflows/` |
| Codex do **front** | `C:\Users\satya\src\financas_bot_telegram` (mesmo worktree do back; territórios disjuntos por pasta) | `feature/fe-*` | UI, componentes, chamadas à API. Pasta de código: `frontend/` |
| Codex de **planejamento** | `C:\Users\satya\src\financas_bot_telegram-planner` (worktree dedicado, fixo em `develop`) | `develop` (commita direto) | Documentação, especificações técnicas, decisões de arquitetura, planos de feature |
| Codex **Reviewer** | qualquer worktree (sessão somente-leitura) | a branch sob revisão | Revisão independente antes do merge (ADR 0005) |
...
- **Codex do back:** código apenas em `financas_bot_telegram/`, `infra/`, `finbot.service`, `.github/workflows/`
- **Codex do front:** código apenas em `frontend/`
- **Codex de planejamento:** commita direto em `develop` e é o **mantenedor** da estrutura de `docs/` (planos, architecture, decisions)
...
- Arquivos da raiz (`AGENTS.md`, `.gitignore`, `TODO.md`) podem ser editados por qualquer instância quando necessário

## Estrutura da pasta docs/

Organizada e mantida pelo Codex de planejamento:
...
Cada dúvida técnica não-trivial que o humano levanta e o Codex de planejamento responde com explicação aprofundada vira um arquivo de resumo em `docs/aprendizado/<topico>.md`. Esses arquivos servem pra o humano revisitar no futuro (refresh memorial, base pra aprofundar, ou referência rápida em outra discussão).

**Comportamento obrigatório do Codex de planejamento:**

Sempre que uma resposta a uma dúvida técnica do humano tiver substância de aprendizado (não é uma pergunta-resposta trivial nem operacional), o Codex **DEVE**, ao final da resposta, criar ou atualizar um arquivo correspondente em `docs/aprendizado/`. Cada arquivo deve conter:
...
|---|---|---|---|
| **Planner** | `C:\Users\satya\src\financas_bot_telegram-planner` | `develop` (sempre) | Codex de planejamento; Cowork do humano pra trabalho em docs |
| **Implementador** | `C:\Users\satya\src\financas_bot_telegram` | branch de feature ativa | Codex do back, Codex do front |
...
1. `C:\Users\satya\src\financas_bot_telegram-planner` — **worktree de trabalho** do planner. Único lugar onde Codex usa `Edit` / `Write` pra modificar arquivos.
2. `C:\Users\satya\src\financas_bot_telegram` — **worktree do implementador**. Montado **somente** pra que o `.git/` real (que mora aqui dentro) fique acessível ao sandbox do Codex, permitindo rodar comandos `git` direto sem precisar pedir ao humano que cole saídas no chat. **Codex NUNCA usa `Edit` / `Write` em paths dentro deste mount.** Se um path de tool começar com `C:\Users\satya\src\financas_bot_telegram\` (sem o sufixo `-planner`), Codex **para e pergunta** antes de prosseguir.
...
### Comandos git que mudam estado — Codex PERGUNTA ANTES via `AskUserQuestion`
...
`git reset --hard`, `git rebase`, `git push --force` (ou `--force-with-lease`), `git stash drop`, `git checkout -- <path>` (sobrescreve trabalho local), qualquer manipulação de reflog/gc/prune. Mesmo com confirmação, Codex descreve o impacto exato antes de executar.

### Race com sessões paralelas

Se houver sessão do Codex do back/front ativa no worktree do implementador (trocando branches), o planner **espera ela terminar** antes de rodar `fetch`/`pull`/`cherry-pick`/`push` pra evitar pegar estado transitório do `.git/` compartilhado. Leitura (`git log`, `git status`) é segura mas pode mostrar HEAD intermediário — interpretar com cautela.
...
```
> **PR integration→develop:** Codex (planner) abre ao fim da sprint; humano revisa e aceita. Este é o único gate humano no fluxo de features.
...
- **Fluxo de PR em dois níveis (atualizado 2026-06-01):**
  ...
  - **integration → develop:** Codex (planner) abre o PR ao fim da sprint; **humano revisa e aceita** — este é o gate de qualidade humano.
...
- **Definição de pronto / pré-merge:** antes de mergear, (1) passar pelos gates do `docs/runbooks/PRE-MERGE-CHECKLIST.md` (build, lint, testes, convenção de branch, território); (2) escrever o status report com frontmatter válido conforme `docs/templates/_TEMPLATE-status.md`; (3) **revisão independente pelo Reviewer** — sessão separada (ADR 0005), obrigatória pra toda task antes do merge (decisão 2026-05-26); (4) **qa-test-specialist** — análise de gaps de cobertura após cada task (ou batch de tasks relacionadas) antes do merge. `estado: concluido` só vale com Reviewer e QA tendo dado ok, todos os gates ok e zero pendência. **Implementadores (back/front) são responsáveis por chamar o Reviewer e o QA e aguardar o ok — não mergeiam sem isso.**
