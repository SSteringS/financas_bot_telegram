...
- **Definição de pronto / pré-merge:** antes de mergear, (1) passar pelos gates do `docs/runbooks/PRE-MERGE-CHECKLIST.md` (build, lint, testes, convenção de branch, território); (2) escrever o status report com frontmatter válido conforme `docs/templates/_TEMPLATE-status.md`; (3) **revisão independente pelo Reviewer** — sessão separada (ADR 0005), **obrigatória pra toda task** antes do merge (decisão 2026-05-26); (4) **qa-test-specialist — condicional, só quando o plano da task pede.** O gate de QA vale quando o plano declara `fluxos_qa` não-vazio (equivalentemente `qa_required: true`); com **`fluxos_qa: []` o round de QA não acontece** e o ciclo é apenas Reviewer → push (**ADR 0019**). `estado: concluido` exige Reviewer ok, todos os gates ok e zero pendência — mais QA ok **quando o gate se aplica**. Quando não se aplica, o status registra o QA como **`not-applicable`** com a justificativa escrita no plano; omitir o registro não vale. **Implementadores (back/front) são responsáveis por chamar o Reviewer sempre, e o QA quando o plano exigir, e aguardar o ok — não mergeiam sem isso.** Limites de rounds (máx. 3 rounds de Reviewer, 2 loops completos Reviewer→QA; estourou, `estado: bloqueado` e escala pro humano): **ADR 0019**.
