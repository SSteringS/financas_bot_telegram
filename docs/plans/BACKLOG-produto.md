---
tipo: backlog-produto
status: ativo
atualizado: 2026-05-29
renomeado_em: "2026-05-29 (era FASE-3-VISUALIZACAO.md)"
audiencia: [planner, arquiteto, PO]
proposito: |
  Backlog de evoluções de produto — features e ideias que aguardam refinamento ou planejamento
  em sprints futuras. Cada item começa como pendência (PO + arquiteto refinam), depois vira
  spec, depois vira tasks BE/FE/DEP.
contem:
  - secao: "Backlog ativo"
    onde: "Linha 'BACKLOG ATIVO' abaixo até o separador histórico"
    proposito: "EVOs candidatas pra próximas sprints (EVO-09 em diante)"
  - secao: "Fase 3d — Evolução pós-MVP (lista original 2026)"
    onde: "Bloco histórico abaixo do separador"
    proposito: "Catálogo de EVOs originalmente listadas (EVO-01 a EVO-07). Várias já promovidas pra sprints — não-tocar como histórico."
  - secao: "Plano original do MVP (Fase 3a/3b/3c)"
    onde: "Bloco histórico — bem abaixo"
    proposito: "Plano que ficou pronto na sprint 01. Referência histórica. Não-tocar."
quando_consultar:
  - "Ao iniciar planning de uma nova sprint — ler 'Backlog ativo' pra ver candidatos prontos."
  - "Ao registrar nova pendência/feature que veio do humano — adicionar no fim do 'Backlog ativo' como EVO-XX."
  - "Ao escrever ADR sobre direção de produto — referenciar a EVO correspondente."
nao_confundir_com:
  - "BACKLOG-evolucao-workflow.md (workflow/processo, não produto)"
  - "PENDENCIAS-TECNICAS.md (débito técnico, não feature)"
  - "docs/sprints/<NN>/plans/ (planos de tarefa concretos da sprint — escopo task-level)"
convencao_de_adicao: |
  Novo item entra em "Backlog ativo" como ## EVO-NN — Título.
  Estrutura mínima: bandeira de status, resumo do humano (na voz dele), sub-frentes identificadas,
  perguntas abertas pro PO/arquiteto, decisões pendentes. Ver EVO-09 como modelo.
referencias_irmas:
  - docs/plans/BACKLOG-evolucao-workflow.md
  - docs/PENDENCIAS-TECNICAS.md
  - docs/sprints/<NN>/README.md
---

# Backlog de produto

Lista viva de **evoluções de produto** — features e ideias que aguardam refinamento ou planejamento em sprints futuras. Cada item nasce como pendência (resumo na voz do humano), depois passa por refinamento com o arquiteto (perguntas respondidas, spec escrita), depois vira tasks BE/FE/DEP numa sprint.

Histórico: este arquivo foi originalmente o plano do MVP (Fase 3 — Camada de Visualização, 2026). O MVP foi entregue (sprint 01) e o documento evoluiu pra catálogo de **evoluções pós-MVP**. As seções "Fase 3a/3b/3c" e "Fase 3d (lista original)" ficam preservadas abaixo como **referência histórica não-editável**. Itens novos entram na seção "Backlog ativo".

---

## Backlog ativo

> Aqui é onde **entram novos itens** vindos do PO. Cada um começa como pendência. Estrutura mínima: bandeira de status, resumo do humano, sub-frentes identificadas, perguntas abertas pro PO/arquiteto, decisões pendentes. Ver EVO-09 abaixo como modelo.

> 🚧 Itens ativos atualmente:
> - EVO-09 — Folha de pagamento (cadastro de funcionários + vales) — ver no bloco "Fase 3d" abaixo onde foi adicionado em 2026-05-29 seguindo a numeração canônica.
>
> Quando começar uma sprint, **promover o item daqui pro `docs/sprints/<NN>/README.md`** (frente da sprint) e abrir o plano correspondente em `docs/sprints/<NN>/plans/`. Manter referência cruzada.

---

## Histórico — Plano original do MVP (Fase 3, 2026)

> ⚠️ **Não editar** a partir deste ponto. Conteúdo abaixo é o plano original do MVP de visualização entregue na sprint 01. Preservado pra rastreabilidade. As subseções "Fase 3d — Evolução pós-MVP" contêm o catálogo original de EVO-01..EVO-09 — esse catálogo continua ativo (novas EVOs são adicionadas seguindo a numeração).

---

# Fase 3 — Camada de Visualização (plano original, MVP — sprint 01)
...
## EVO-09 — Folha de pagamento (cadastro de funcionários + vales)

> 📥 **Pendência de produto** — registrada 2026-05-29 pelo humano. **Aguardando refinamento com o arquiteto** antes do planning da próxima sprint. Não está planejado nem dimensionado; o resumo abaixo é o ponto de partida da discovery.

**Resumo do humano:** o front ganha uma **aba nova de "Folha de pagamento"** (separada da aba atual de "Pedidos de pagamento"), onde o usuário pode:

1. **Cadastrar funcionários** — nome, salário, forma de pagamento.
2. **Forma de pagamento por funcionário:**
   - **PIX** → guardar a chave (CPF/celular/email/aleatória).
