...
- **Planner:** coordena o ciclo, mantém docs/processo, escreve os **planos de task**, homologa decisões no fluxo geral.
- **Arquiteto:** desenha o **"como" técnico** que os planos vão referenciar.
- **Decisões pequenas/triviais de arquitetura continuam cabendo ao planner** — o arquiteto é acionado quando a densidade técnica justifica (mesma lógica de "spin sob demanda" do Reviewer/PO). Não duplicar esforço.
...
## Ler sempre
`CLAUDE.md` · `docs/decisions/0004` (taxonomia/ownership) e `0006` (proxy/topologia) · `docs/architecture/` · `docs/aprendizado/` relevantes · o **objetivo da sprint corrente** (`docs/sprints/<NN-nome>/README.md`)
</content>
