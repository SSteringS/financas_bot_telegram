...
A solução já estava implementada na prática — `docs/status/_TEMPLATE.md` (frontmatter YAML obrigatório) + `docs/runbooks/PRE-MERGE-CHECKLIST.md` (gates verificáveis) — mas a **decisão** por trás dela nunca foi registrada como ADR. Este registro a torna canônica.
...
1. **Schema obrigatório (frontmatter YAML parseável)** em todo `docs/status/<TASK-ID>-*.md`, conforme `docs/status/_TEMPLATE.md`: `task`, `branch`, `responsavel`, `estado` (`concluido`|`parcial`|`bloqueado`), bloco `gates:` (`build`, `lint`, `testes`, `testes_total`, `testes_novos`, `branch_convencao`, `territorio`), `desvios`, `pendencias_humano`.
...
- **Agregável:** frontmatter YAML deixa um script ler todos os `docs/status/*.md` e montar um painel do projeto (tasks por estado, desvios por área, soma de testes, gate-fails) sem esforço.
...
- `docs/status/_TEMPLATE.md` (o schema)
