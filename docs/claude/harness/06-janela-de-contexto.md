<!-- Fonte: https://code.claude.com/docs/pt/context-window | Baixado em: 2026-08-06 | slug: context-window -->
...
* **Antes de você digitar qualquer coisa**: CLAUDE.md, memória automática, nomes de ferramentas MCP e descrições de skills são todos carregados no contexto. Sua própria configuração pode adicionar mais aqui, como um [estilo de saída](https://code.claude.com/docs/pt/output-styles) ou texto de [`--append-system-prompt`](https://code.claude.com/docs/pt/cli-reference), que ambos vão para o prompt do sistema da mesma forma.
* **Conforme Claude trabalha**: cada leitura de arquivo adiciona ao contexto, [regras com escopo de caminho](https://code.claude.com/docs/pt/memory#path-specific-rules) são carregadas automaticamente junto com arquivos correspondentes, e um [hook PostToolUse](https://code.claude.com/docs/pt/hooks-guide) é acionado após cada edição.
* **O prompt de acompanhamento**: um [subagent](https://code.claude.com/docs/pt/sub-agents) lida com a pesquisa em sua própria janela de contexto separada, então as leituras de arquivo grandes ficam fora da sua. Apenas o resumo e um pequeno trailer de metadados voltam.
...
Quando uma sessão longa é compactada, Claude Code resume o histórico de conversa para caber na janela de contexto. A partir da v2.1.198, a solicitação de resumo herda a configuração de [extended thinking](https://code.claude.com/docs/pt/model-config#extended-thinking) da sua sessão, portanto ela raciocina com o thinking habilitado quando sua sessão o tem habilitado e permanece desativado caso contrário. O thinking afeta apenas como o resumo é produzido; suas configurações de sessão permanecem inalteradas depois. O que acontece com suas instruções depende de como foram carregadas:
...
* **Delegue leituras grandes**: envie pesquisa para um [subagent](https://code.claude.com/docs/pt/sub-agents) para que o conteúdo do arquivo fique em sua janela de contexto, não na sua.

Se você precisar de uma janela maior em vez de uma conversa menor, Fable 5, Sonnet 5, Opus 4.6 e posteriores, e Sonnet 4.6 suportam uma janela de contexto de 1 milhão de tokens. Veja [Extended context](https://code.claude.com/docs/pt/model-config#extended-context) para disponibilidade por plano e como selecionar uma variante de modelo `[1m]`. Sonnet 5 é executado em 1M sem variante `[1m]` para selecionar; veja [Sonnet 5 context window](https://code.claude.com/docs/pt/model-config#sonnet-5-context-window) para seus limites de auto-compactação e a exceção do gateway LLM. A compactação funciona da mesma forma no limite maior.
...
* [Estenda Claude Code](https://code.claude.com/docs/pt/features-overview): quando usar CLAUDE.md vs skills vs regras vs hooks vs MCP
* [Armazene instruções e memórias](https://code.claude.com/docs/pt/memory): hierarquia CLAUDE.md e memória automática
* [Subagents](https://code.claude.com/docs/pt/sub-agents): delegue pesquisa a uma janela de contexto separada
* [Melhores práticas](https://code.claude.com/docs/pt/best-practices): gerenciando contexto como sua restrição principal
* [Prompt caching](https://code.claude.com/docs/pt/prompt-caching): quais ações invalidam o prefixo em cache
* [Reduza o uso de tokens](https://code.claude.com/docs/pt/costs#reduce-token-usage): estratégias para manter o uso de contexto baixo
